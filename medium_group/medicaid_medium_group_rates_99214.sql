-- Medicaid Provider Rate Distributions by State - Medium Groups Only
-- Analysis of office visit code 99214 with dual-eligible filtering (2023-2024 only)
-- Filters to medium group providers (TIN size 10-99) EXCLUDING health systems
...
-- Step 2: Get medium group providers (TIN size 10-99) excluding health systems
medium_group_providers AS (
  SELECT
    npi,
    tin,
    tin_size,
...
-- Step 3: Filter Medicaid data to medium group providers only
provider_rates AS (
  SELECT
  ...
  JOIN medium_group_providers mg ON mps.billing_provider_npi_num = mg.npi  -- Medium group filter
...
-- Notes:
-- - Medium groups defined as TIN size 10-99 providers (from payermrf.npi_to_tin)
