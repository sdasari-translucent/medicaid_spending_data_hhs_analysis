-- Medicaid Provider Rate Distributions by State - Large Groups Only
-- Analysis of office visit code 99214 with dual-eligible filtering (2023-2024 only)
-- Filters to large group providers (TIN size 100+) EXCLUDING health systems
...
-- Step 2: Get large group providers (TIN size 100+) excluding health systems
large_group_providers AS (
  SELECT
    npi,
    tin,
    tin_size,
...
provider_rates AS (
  ...
  JOIN large_group_providers lg ON mps.billing_provider_npi_num = lg.npi  -- Large group filter
...
-- Notes:
-- - Large groups defined as TIN size 100+ providers (from payermrf.npi_to_tin)
